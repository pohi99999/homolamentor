...
const CTP_CONTACT_NOTE = '2026-07-21: Érdeklődik a Győrszentiván és Kamion Park M5 iránt. Teaserek kiküldve, átirányítva Homola Lászlóhoz műszaki egyeztetésre.';
const CTP_MASTER_NOTE = '2026-07-21: CTP érdeklődik (Tar Péter) a Győrszentiván és Kamion Park M5 projektek iránt. Teaserek kiküldve, átirányítva Homola Lászlóhoz műszaki egyeztetésre.';
...
function processContactsCRM() {
  console.log('\n--- 1. CONTACTS CRM (CONTACTS) feldolgozása ---');
  ...
  rows.forEach((row, idx) => {
    ...
    const isMatch = name.toLowerCase().includes('péter tar') ||
                    name.toLowerCase().includes('peter tar') ||
                    company.toLowerCase().includes('ctp') ||
                    rowStr.includes('ctp');

    if (isMatch) {
      const rowIndex = idx + 1;
      console.log(`[Contacts CRM] CTP / Péter Tar sor azonosítva (${rowIndex}. sor): ${name} (${company})`);

      // 1. Statusz -> "Válaszolt / Érdeklődik"
      dataUpdates.push({
        range: `CONTACTS!${colLetter(statusColIdx)}${rowIndex}`,
        values: [['Válaszolt / Érdeklődik']]
      });
      ...
      // 3. Megjegyzes -> befűzve
      const currentNote = row[noteColIdx] || '';
      let newNote = CTP_CONTACT_NOTE;
      if (currentNote && !currentNote.includes(CTP_CONTACT_NOTE)) {
        newNote = `${currentNote} | ${CTP_CONTACT_NOTE}`;
      }
      dataUpdates.push({
...
function processMasterCRM() {
  ...
  let dateColIdx = headers.findIndex(h => /utolso_interakcio|utolso_inmail_datum|első_kapcsolatfelvétel_dátuma/i.test(h));
  ...
  rows.forEach((row, idx) => {
    ...
    // Matching CTP or Győrszentiván / Kamion Park M5 with CTP
    const isMatch = company.toLowerCase().includes('ctp') ||
                    contact.toLowerCase().includes('tar') ||
                    contact.toLowerCase().includes('gondi') ||
                    ((rowStr.includes('győrszentiván') || rowStr.includes('gyorszentivan') || rowStr.includes('kamion park')) && rowStr.includes('ctp'));

    if (isMatch) {
      const rowIndex = idx + 1;
      console.log(`[Master CRM] CTP / Projekt sor azonosítva (${rowIndex}. sor): ${company} (${category}) - ${contact}`);

      // 1. Aktuális_Státusz -> "Tárgyalás alatt"
      dataUpdates.push({
        range: `Master_Vevőlista!${colLetter(statusColIdx)}${rowIndex}`,
        values: [['Tárgyalás alatt']]
      });
      ...
      // 3. Megjegyzés -> befűzve: CTP érdeklődik (Tar Péter)
      const currentNote = row[noteColIdx] || '';
      let newNote = CTP_MASTER_NOTE;
      if (currentNote && !currentNote.includes(CTP_MASTER_NOTE)) {
        newNote = `${currentNote} | ${CTP_MASTER_NOTE}`;
      }
      dataUpdates.push({
...
function main() {
  console.log('=== CTP HUNGARY ÉRDEKLŐDÉS FRISSÍTÉSE (CRM) ===');
  processContactsCRM();
  processMasterCRM();
  console.log('\n🎉 CTP Hungary (Tar Péter) érdeklődése sikeresen rögzítve mindkét CRM táblázatban!');
}
